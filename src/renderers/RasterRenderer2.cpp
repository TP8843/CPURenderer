...
float** RasterRenderer2::generateDepthBuffer(const std::vector<ModelTriangle>& triangles, const size_t width, const size_t height) const
{
...
CanvasPoint RasterRenderer2::projectVertexOntoCanvasPoint(const glm::vec3 vertex, const size_t width,
                                                          const size_t height) const
{
    ...
    if (vertex.z != 0)
    {
        u = camera.imagePlaneScaling * camera.focalLength * (-vertex.x / vertex.z) + (static_cast<float>(width) / 2);
        v = camera.imagePlaneScaling * camera.focalLength * (vertex.y / vertex.z) + (static_cast<float>(height) / 2);
